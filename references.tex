\section{References}
\vspace{0.2cm}

\vspace{0.2cm}
\noindent
[1] Satoshi Nakamoto,  {\em Bitcoin: A Peer-to-Peer Electronic Cash System }, 2009, available from \path|https://bitcoin.org/bitcoin.pdf|.

\vspace{0.2cm}
\noindent
[2] Sunny King, Scott Nadal,  {\em PPCoin: Peer-to-Peer Crypto-Currency with Proof-of-Stake }, 19.08.2012, available from \path|https://peercoin.net/assets/paper/peercoin-paper.pdf|.

\vspace{0.2cm}
\noindent
[3] Rob Halfoerd,  {\em Crypto-Currency using Berkeley Open Infrastructure Network
Computing Grid as a Proof Of Work }, 23.05.2014, available from \path|https://coss.io/documents/white-papers/Gridcoin.pdf|.

\vspace{0.2cm}
\noindent
[4] {\em Gridcoin}, Rewarding Volunteer Distributed Computing 2014-2017, available from \path|http://www.Gridcoin.us|.

\vspace{0.2cm}
\noindent
[5] {\em BOINC, Open-source software for volunteer computing }, 2002, available from \path|http://boinc.berkeley.edu|.

\vspace{0.2cm}
\noindent
[6] {\em Gridcoin Entry on Wikipedia }, 30.08.2016, available from \path|http://en.wikipedia.org/wiki/Gridcoin|.

\vspace{0.2cm}
\noindent
[7] Andreas M. Antonopoulos, {\em Mastering Bitcoin }, O'Reilly, 01.06.2017.

\vspace{0.2cm}
\noindent
[8] Aleksander Berentsen, Fabian Schuer, {\em Bitcoin, Blockchain und Kryptoassets }, Universitaet Basel, 2017.

\vspace{0.2cm}
\noindent
[9] Roger Wattenhofer, {\em The Science of the Blockchain }, Inverted Forest Publishing, 2016.

\vspace{0.2cm}
\noindent
[10] Devin Williams, {\em Cryptocurrency Compendium: A Reference for Digital Currencies }, Darknetreferences llc, 22.06.2017.

\vspace{0.2cm}
\noindent
[11] {\em World Community Grid }, available from  \path|http://www.worldcommunitygrid.org|.

\vspace{0.2cm}
\noindent
[12] {\em GPUGRID.net }, available from  \path|http://www.gpugrid.net|.

\vspace{0.2cm}
\noindent
[13] {\em Rosetta@home }, available from  \path|https://boinc.bakerlab.org/|.

\vspace{0.2cm}
\noindent
[14] {\em Asteroids@home }, available from  \path|http://asteroidsathome.net/boinc/|.

\vspace{0.2cm}
\noindent
[15] {\em Climate Prediction.net }, available from  \path|http://www.climateprediction.net|.

\vspace{0.2cm}
\noindent
[16] {\em yoyo@home }, available from  \path|www.rechenkraft.net/yoyo/|.

\vspace{0.2cm}
\noindent
[17] {\em Collatz Conjecture }, available from  \path|http://boinc.thesonntags.com/collatz/|.

\vspace{0.2cm}
\noindent
[18] {\em LHC@home Classic }, CERN, Geneva, available from  \path|http://lhcathome.cern.ch|.

\vspace{0.2cm}
\noindent
[19] {\em Einstein@home }, available from  \path|http://einsteinathome.org|.

\vspace{0.2cm}
\noindent
[20] {\em Seti@home }, Berkeley, University of California, since 1998, available from  \path|http://setiathome.berkeley.edu|.

\vspace{0.2cm}
\noindent
[21] {\em On gridcoin mining, profitability, value and other random thoughts. }, hotbit, September 2017, available from  \path|https://steemit.com/gridcoin/@hotbit/on-gridcoin-mining-profitability-value-and-other-random-thoughts|.

\vspace{0.2cm}
\noindent
[22] {\em Cobblestone, Recently Averaged Credit }, entry on BOINC wiki, available from  \path|http://boinc.berkeley.edu/wiki/Computation_credit#Recent_Average_Credit|.

\vspace{0.2cm}
\noindent
[23] {\em Proof-of-Research }, entry on Gridcoin wiki, available from  \path|http://wiki.Gridcoin.us/Proof-of-Research|.

\vspace{0.2cm}
\noindent
[24] {\em boincstats.com }, BAM! account manager and BOINC statistics, available from  \path|http://www.boincstats.com|.

\vspace{0.2cm}
\noindent
[25]  {\em TOP 500 Supercomputers list }, June 2017, available from  \path|https://www.top500.org/lists/2017/06/7|.

\vspace{0.2cm}
\noindent
[26] {\em Bitcoin Charts }, estimation of Bitcoin speed , available from \path|https://bitcoincharts.com/bitcoin/|.

\vspace{0.2cm}
\noindent
[27] {\em Bitcoin Energy Consumption Index}, estimation of Bitcoin power consumption, availabe from \path|http://digiconomist.net/bitcoin-energy-consumption|.

\vspace{0.2cm}
\noindent
[28] {\em Ethereum Energy Consumption Index}, estimation of Ethereum power consumption, availabe from \path|https://digiconomist.net/ethereum-energy-consumption|.

\vspace{0.2cm}
\noindent
[29] {\em How Much Power Does the Bitcoin Network Use}, estimation of Bitcoin power consumption, availabe from \path|https://www.thebalance.com/how-much-power-does-the-bitcoin-network-use-391280|.

\vspace{0.2cm}
\noindent
[30]  {\em How we calculate CO2 impact}, Conversion rate for kWh to CO2 metric tons, October 2017, available from  \path|https://carbonfund.org/how-we-calculate/|

\vspace{0.2cm}
\noindent
[31]  {\em The math behind BOINC RAC}, jefpatat, September 2017, available from  \path|https://steemit.com/Gridcoin/@jefpatat/the-math-behind-boinc-rac/|

\vspace{0.2cm}
\noindent
[32]  {\em RAC and Gridcoin rewards for dummies}, hotbit, September 2017, available from \path|https://steemit.com/Gridcoin/@hotbit/rac-and-grc-rewards-for-dummies-calculate-you-maximum-rac-in-5-seconds|

\vspace{0.2cm}
\noindent
[33] {\em Idea behind Einsteinium}, Einsteinium foundation, October 2017, available from \path|https://www.emc2.foundation/more-about-foundation|

\vspace{0.2cm}
\noindent
[34] {\em Science Power and Research Coin (SPARC) Whitepaper}, October 2017, \path|http://sparc.network/sparc-whitepaper-4.pdf|

\vspace{0.2cm}
\noindent
[35] {\em Golem Network}, October 2017, \path|http://golem.network/|

\vspace{0.2cm}
\noindent
[36] {\em BURP the Big and Ugly Rendering Project}, 2004-2017, \path|http://burp.renderfarming.net/|

\vspace{0.2cm}
\noindent
[37] {\em GPU - a Global Processing Unit}, 2004-2014, \path|http://gpu.sourceforge.net|

\vspace{0.2cm}
\noindent
[38] {\em Solarcoin - A global rewards program for solar electricity generation }, \path|http://solarcoin.org|

\vspace{0.2cm}
\noindent
[39] {\em 21 Terms to Understand Cryptocurrency }, Max Middelmann, October 2017, \path|https://medium.com/the-mission/21-terms-to-understand-cryptocurrency-8bee30aa8dfc|

\vspace{0.2cm}
\noindent
[40] {\em Transaction}, Bitcoin Wiki, \path|http://en.bitcoin.it/wiki/Transaction|

\vspace{0.2cm}
\noindent
[41] {\em Google Definitions}, \path|www.google.com|

\vspace{0.2cm}
\noindent
[42] {\em Gridcoin Project Whitelist}, \path|http://www.Gridcoin.us/Guides/whitelist.htm|

\vspace{0.2cm}
\noindent
[43] {\em Novacoin}, \path|http://coinwiki.info/en/Novacoin|

\vspace{0.2cm}
\noindent
[44] {\em Blackcoin Proof of Stake Protocol v2}, Pavel Vasin, \path|https://blackcoin.co/blackcoin-pos-protocol-v2-whitepaper.pdf|

\vspace{0.2cm}
\noindent
[45] {\em Zerocash: Decentralized Anonymous Payments from Bitcoin (extended version)}, Eli Ben-Sasson et. al, May 18 2014, \path|http://zerocash-project.org/media/pdf/zerocash-extended-20140518.pdf|

\vspace{0.2cm}
\noindent
[46] {\em Kimoto’s Gravity Well}, Cryptorials, \path|http://cryptorials.io/glossary/kimotos-gravity-well/|

\vspace{0.2cm}
\noindent
[47] {\em Gridcoin - Rewarding research instead of mining}, Anonymous User, \path|https://docs.google.com/document/d/1PUmLpL3jb-Dhf9-60MPpCYpCvBukUIWeKO8Pww0bhII/edit#heading=h.1gqiga6cbzja|

\vspace{0.2cm}
\noindent
[48] {\em Plagiarism and falsified data slip into the scientific literature: a report}, John Timmer, Ars Technica, 08.08.2007, \path|https://arstechnica.com/features/2007/08/plagiarism-and-falsified-data-slip-into-the-scientific-literature/|

\vspace{0.2cm}
\noindent
[49] {\em Can we trust peer review? New study highlights some problems}, Cathleen O'Grady, Ars Technica, 20.07.2016, \path|https://arstechnica.com/science/2016/07/can-we-trust-peer-review-new-study-highlights-some-problems/|

\vspace{0.2cm}
\noindent
[50] {\em Mindless Eating or how to send an entire life of research into question}, Cathleen O'Grady, Ars Technica, 24.04.2017, \path|https://arstechnica.com/science/2017/04/the-peer-reviewed-saga-of-mindless-eating-mindless-research-is-bad-too/|

\vspace{0.2cm}
\noindent
[51] {\em Do we need an alternative to peer-reviewed journals?}, Jonathan M. Gitlin, Ars Technica, 18.07.2011 \path|https://arstechnica.com/science/2011/07/do-we-need-an-alternative-to-peer-reviewed-journals/|

\vspace{0.2cm}
\noindent
[52] {\em Open access: All human knowledge is there, why can not everybody access it?}, Glyn Moody, Ars Technica, 06.07.2016, \path|https://arstechnica.co.uk/science/2016/06/what-is-open-access-free-sharing-of-all-human-knowledge/|

\vspace{0.2cm}
\noindent
[53] {\em There is a debate raging in science about what should count as significant}, Cathleen O'Grady, Ars Technica, 04.08.2017, \path|https://arstechnica.com/science/2017/08/theres-a-debate-raging-in-science-about-what-should-count-as-significant/|

\vspace{0.2cm}
\noindent
[54] {\em Carbon Footprint Calculator}, Carbon Footprint Ltd, Hampshire UK, October 2017 \path|https://calculator.carbonfootprint.com/calculator.aspx?tab=4|

\vspace{0.2cm}
\noindent
[55] {\em grcpool - Gridcoin Mining Pool}, \path|www.grcpool.com|

\vspace{0.2cm}
\noindent
[56] {\em gridcoinresearchd - RPC commands}, \path|http://wiki.gridcoin.us/RPC_commands|

\vspace{0.2cm}
\noindent
[57] {\em Gridcoin Stats Polls - Active and Past Pools}, \path|http://www.gridcoinstats.eu/poll|

\vspace{0.2cm}
\noindent
[58] {\em A Single Bitcoin Transaction Takes Thousands of Times More Energy Than a Credit Card Swipe }, Christopher Malmo, 07.05.2017, \path|https://motherboard.vice.com/en_us/article/ypkp3y/bitcoin-is-still-unsustainable|

\vspace{0.2cm}
\noindent
[59] {\em Breaking and Fixing Gridcoin}, Martin Grothe, Tobias Niemann, Juraj Somorovsky, Joerg Schwenk, Horst Goertz Institute for IT Security, Ruhr University Bochum, 16.09.2016, \path|https://www.usenix.org/system/files/conference/woot17/woot17-paper-grothe.pdf|

\vspace{0.2cm}
\noindent
[60] {\em Gridcoin PoS kernel v8}, TomasBrod, 12.09.2017, \path|https://github.com/gridcoin/Gridcoin-Research/wiki/Stake-V8|

\vspace{0.2cm}
\noindent
[61] {\em DEV Neural Network}, TomasBrod, 31.07.2017, \path|https://github.com/gridcoin/Gridcoin-Research/wiki/DEV-Neural-Network|

\vspace{0.2cm}
\noindent
[62] {\em Gridcoin V8 difficulty and what it tells you}, skcin, September 2017, \path|https://steemit.com/gridcoin/@skcin/gridcoin-v8-difficulty-and-what-it-tells-you|

\vspace{0.2cm}
\noindent
[63] {\em Bitcoin Kernel Target}, Bitcoin Development Team, \path|https://en.bitcoin.it/wiki/Target|

\vspace{0.2cm}
\noindent
[64] {\em Decentralised Currencies Are Probably Impossible But Let’s At Least Make Them Efficient}, Ben Laurie, 05.07.2011, \path|http://www.links.org/files/decentralised-currencies.pdf|

\vspace{0.2cm}
\noindent
[65]  {\em On Bitcoin and Red Balloons}, Barbaioff M. et al., 2011, \path|https://arxiv.org/pdf/1111.2626.pdf|




